...
namespace estimators {
    ...
    double GroebnerDivisionModelEstimator::computeErrorsAndEstimateQuantile(double distortion_coefficient,
        ...
        utils::computeErrors<functors::EpipolarCurveDistanceError, scene::StandartDivisionModelStereoPair, double>
                (stereo_pair_, left_residuals_, right_residuals_, options_.image_radius);
        for (std::size_t k = 0; k < number_of_points_; ++k) {
            errors[k] = std::abs(left_residuals_[k]) + std::abs(right_residuals_[k]);
        }
        return utils::estimateQuantile(errors, options_.quantile_to_minimize);
    }
    ...
    void GroebnerDivisionModelEstimator::estimateImpl() {
        ...
        scene::ImagePoints u1d_ = stereo_pair_.getLeftKeypoints();
        scene::ImagePoints u2d_ = stereo_pair_.getRightKeypoints();
